...
@views function update_halo!(A, neighbors_x, comm)
    ...
    if neighbors_x[2] != MPI.PROC_NULL
        ...
        sendbuf = A[end-1]
        recvbuf = zeros(1) # receive and put at position end

        # send local [end-1] to neighbour [2]
        MPI.Send(sendbuf, neighbors_x[2], 1, comm)

        # receive local [end] from neighbour [2]
...
@views function diffusion_1D_mpi(; do_save=false)
    ...
    neighbors_x = MPI.Cart_shift(comm_cart, 0, 1)   # stores the index of the left and right neighbour, for 0 => (-1, 1)
    ...
    if do_save 
        file = matopen("$(@__DIR__)/mpi1D_out_C_$(me).mat", "w"); write(file, "C", Array(C)); close(file) 
        file = matopen("$(@__DIR__)/mpi1D_out_xc_$(me).mat", "w"); write(file, "xc", Array(xc)); close(file) 
    end
