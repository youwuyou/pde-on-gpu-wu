...
@views function porous_convection_2D()
    ...
    anim = @animate for it = 1:nt
        ...
        while err_Pf >= ϵtol && iter <= maxiter
            ...
            # darcy flux - 99 x 50
            qDx[2:end-1, :]  .-= (qDx[2:end-1, :] .+ k_ηf.* (diff(Pf, dims=1) ./ dx + αρg .* (1 .- (T[1:end-1, :] .- T_init[1:end-1, :])))) ./ (1.0 + θ_dτ)
            qDy[:, 2:end-1]  .-= (qDy[:, 2:end-1] .+ k_ηf.* (diff(Pf, dims=2) ./ dy + αρg .* (1 .- (T[:, 1:end-1] .- T_init[:, 1:end-1])))) ./ (1.0 + θ_dτ)
        ...
        # For temperature flux
        dt_adv = ϕ*min(dx/maximum(abs.(qDx)), dy/maximum(abs.(qDy)))/2.1
        dt     = min(dt_diff,dt_adv)
        
        # 98 x 48
        # 100x50 <-> diff(qDx, dims=1)
        # 100x50 <-> diff(qDy, dims=2)
        T[2:end-1,2:end-1]             .+= dt.*λ_ρCp.*( (T[2:end-1,2:end-1] .- T_old[2:end-1,2:end-1]) ./ dt 
                                                               .+ (diff(qDx, dims=1)[2:end-1, 2:end-1] ./ dx).+
                                                                  (diff(qDy, dims=2)[2:end-1, 2:end-1] ./ dy)
                                                      ) # diffusive

        # advection term using upwinding scheme
        (vx > 0) && (T[2:end,   1:end] .-= dt ./ ϕ .* (diff(T, dims=1)./dx .* qDx[2:end-1,:]))
        (vx < 0) && (T[1:end-1, 1:end] .-= dt ./ ϕ .* (diff(T, dims=1)./dx .* qDx[2:end-1,:]))

        (vy > 0) && (T[1:end,   2:end] .-= dt ./ ϕ .* (diff(T, dims=2)./dy .* qDy[:,2:end-1]))
        (vy < 0) && (T[1:end, 1:end-1] .-= dt ./ ϕ .* (diff(T, dims=2)./dy .* qDy[:,2:end-1]))
    ...
    # file I/O
    gif(anim, "transient_momentum_eq_pressure_2D.gif", fps = 50)
end
