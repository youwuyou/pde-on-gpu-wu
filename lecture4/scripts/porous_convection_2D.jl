...
@views function porous_convection_2D()
    ...
    anim = @animate for it = 1:nt
        ...
        while err_Pf >= ϵtol && iter <= maxiter
            ...
            # darcy flux
            qDx[2:end-1, :]  .-= (qDx[2:end-1, :] .+ k_ηf.* (diff(Pf, dims=1) ./ dx + αρg .* T[1:end-1,:])) ./ (1.0 + θ_dτ)
            qDy[:, 2:end-1]  .-= (qDy[:, 2:end-1] .+ k_ηf.* (diff(Pf, dims=2) ./ dy + αρg .* T[:,1:end-1])) ./ (1.0 + θ_dτ)
        ...
        T                 .+= dt.*λ_ρCp.*((diff(qTx, dims=1)./ dx) .+ (diff(qTy, dims=2)./ dy))

        # PART II:  
        # advection term - using upwinding scheme
        T[2:end,   :]     .-= dt ./ ϕ .* (diff(T, dims=1)./dx .* qDx[2:end-1,:])
        T[1:end-1, :]     .-= dt ./ ϕ .* (diff(T, dims=1)./dx .* qDx[2:end-1,:])

        T[1:end,   2:end] .-= dt ./ ϕ .* (diff(T, dims=2)./dy .* qDy[:,2:end-1])
        T[1:end, 1:end-1] .-= dt ./ ϕ .* (diff(T, dims=2)./dy .* qDy[:,2:end-1])
