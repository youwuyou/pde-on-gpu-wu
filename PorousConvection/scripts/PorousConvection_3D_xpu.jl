...
@parallel function compute_flux_temp!(Pf, T, qTx, qTy, qTz, _dx, _dy, _dz, λ_ρCp, _1_θ_dτ_T, T_inn_xy, T_inn_yz, T_inn_xz)

    # qTx[ix,iy]  -= (qTx[ix,iy] + λ_ρCp*(@d_xa(T[:,2:end-1])* _dx)) * _1_θ_dτ_T                    
    # qTy[ix,iy]  -= (qTy[ix,iy] + λ_ρCp*(@d_ya(T[2:end-1,:])* _dy)) * _1_θ_dτ_T

    @all(T_inn_xy) = @inn_xy(T)
    @all(T_inn_yz) = @inn_yz(T)
    @all(T_inn_xz) = @inn_xz(T)

    @all(qTx)  = @all(qTx) - (@all(qTx) + λ_ρCp* @d_xa(T_inn_yz) * _dx) * _1_θ_dτ_T                    
    @all(qTy)  = @all(qTy) - (@all(qTy) + λ_ρCp* @d_ya(T_inn_xz) * _dy) * _1_θ_dτ_T

    # FIXME: check what the T_inn_z shall be
    @all(qTz)  = @all(qTz) - (@all(qTz) + λ_ρCp* @d_za(T_inn_xy) * _dz) * _1_θ_dτ_T
...
function compute!(Pf, T, T_old, qDx, qDy, qDz, qTx, qTy, qTz, dTdt, _dx, _dy, _dz, _dt, k_ηf, αρg, _ϕ, _1_θ_dτ_D, _β_dτ_D, λ_ρCp, _1_θ_dτ_T, _dt_β_dτ_T, T_inn_xy, T_inn_yz, T_inn_xz)
    ...
    # thermo
    # @parallel compute_flux_temp!(Pf, T, qTx, qTy, qTz, _dx, _dy, _dz, λ_ρCp, _1_θ_dτ_T, T_inn_xy, T_inn_yz, T_inn_xz)
...
@views function porous_convection_3D_xpu(nz_, nt_, nvis_; do_visu=false, do_check=true, test=true)
    ...
    T_inn_yz     = @zeros(nx-2, ny, nz)
    T_inn_xz     = @zeros(nx, ny-2, nz)
    T_inn_xy     = @zeros(nx, ny, nz-2)
    ...
    for it = 1:nt
        ...
        while max(err_D,err_T) >= ϵtol && iter <= maxiter
            ...
            compute!(Pf, T, T_old, qDx, qDy, qDz, qTx, qTy, qTz, dTdt, _dx, _dy, _dz, _dt, k_ηf, αρg, _ϕ, _1_θ_dτ_D, _β_dτ_D, λ_ρCp, _1_θ_dτ_T, _dt_β_dτ_T, T_inn_xy, T_inn_yz, T_inn_xz)
        ...
        if it % nvis == 0
            ...
            qDy_c ./= qDmag
            qDz_c ./= qDmag
            qDx_p = qDx_c[1:st:end,1:st:end,1:st:end]
            qDy_p = qDy_c[1:st:end,1:st:end,1:st:end]
            qDz_p = qDz_c[1:st:end,1:st:end,1:st:end]
    ...
    A_eff = (9 * 2 + 2) / 1e9 * nx * ny * nz * sizeof(Float64)  # Effective main memory access per iteration [GB]
